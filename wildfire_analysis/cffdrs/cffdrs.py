...
def ffmc_calc(tas, pr, sfcWind, hurs, ffmc0):
    ...
    k1 = 0.424 * (1.0 - ((100.0 - hurs) / 100.0)**1.7) + \
        0.0694 * np.sqrt(sfcWind) * (1.0 - ((100.0 - hurs) / 100.0)**8) # Eq. 7a
...
def cffdrs_calc(tas,pr,sfcWind,hurs,mon,ffmc0=85.0,dmc0=6.0,dc0=15.0):
    ...
    for i in range(ndays):
        ...
        isi[i,...] = isi_calc(ffmc,sfcWind[i,...])
        bui[i,...] = bui_calc(dmc,dc)
        fwi[i,...] = fwi_calc(isi,bui)
        dsr[i,...] = dsr_calc(fwi)
